...
RUN sudo apt-get -q update && \
    ...
    firefox remmina tint2 iputils-ping dnsutils \
    gcc make build-essential wget curl unzip apt-utils xz-utils libkrb5-dev gradle libpulse0 android-tools-adb android-tools-fastboot \
    gnome-terminal software-properties-common apt-transport-https wget language-pack-zh* language-pack-ja language-pack-ko && \
    sudo apt-get remove -y --purge openjdk-*-jdk && \
